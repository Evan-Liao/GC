...
export interface ActionReducer<T> {
    (state: T | undefined, action: Action): T | string;
}

export const skipReduceResult = '__skip_reduce__';
...
export default class XStateSubject<T> extends BehaviorSubject<T> {
    ...
    constructor(initialState: T) {
        ...
        const stateAndAction$: Observable<{ state: any; action: Action; skip?: boolean }> = scan.call(actionsOnQueue$, this.reduceState.bind(this), { state: initialState });

        this.stateSubscription = stateAndAction$.subscribe(({ state, action, skip }) => {
            skip !== true && super.next.call(this, state);
    ...
    private reduceState(
        ...
        if (!reducer) {
            ...
            let tempReduceResult = reducer(state, action);
            if (tempReduceResult === skipReduceResult) {
                reduceResult = state;
                skip = true;
            } else {
                reduceResult = tempReduceResult;
            }
        }

        return { state: reduceResult, action, skip };
    }
    ...
    public dispatch(action: Action): Observable<T> {
        ...
        _debugger.flag('XStateSubject', 'dispatch', action);
